...
async def send_invoice(invoice_id: str, current_user=Depends(get_current_user)):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        resp = await client.post(
            ...
            json={
                "from": f"{business.get('business_name', 'ReceiptVault')} <invoices@dealdily.com>",
                ...
                <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
                  ...
                  <div style="background:#f2faf5;padding:24px 32px">
                    ...
                    <p style="color:#5c5c58">Please find your invoice attached. The total amount due is <b style="color:#1a6b3a;font-size:18px">${invoice['total']:.2f}</b>.</p>
                    <table style="width:100%;border-collapse:collapse;margin:16px 0">
                      <tr style="background:#1a6b3a;color:white">
                        <th style="padding:8px 12px;text-align:left">Description</th>
                        <th style="padding:8px 12px;text-align:right">Amount</th>
                      </tr>
                      {''.join(f'<tr style="background:{"white" if i%2==0 else "#e8f4ec"}"><td style="padding:8px 12px">{item["description"]}</td><td style="padding:8px 12px;text-align:right">${item["amount"]:.2f}</td></tr>' for i,item in enumerate(invoice['items']))}
                      <tr><td style="padding:8px 12px;text-align:right;color:#5c5c58" colspan="1">Total</td><td style="padding:8px 12px;text-align:right;font-weight:bold;color:#1a6b3a;font-size:16px">${invoice['total']:.2f}</td></tr>
                    </table>
                    {"<p style='color:#5c5c58'><b>Due Date:</b> " + invoice.get('dueDate','') + "</p>" if invoice.get('dueDate') else ""}
                    {"<p style='color:#5c5c58'><b>Notes:</b> " + invoice.get('notes','') + "</p>" if invoice.get('notes') else ""}
